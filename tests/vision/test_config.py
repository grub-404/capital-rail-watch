...
def test_defaults(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("VISION_SCREENSHOT_MIN_VIDEO_SEC", raising=False)
    ...
    assert c.screenshot_min_video_sec == 6.0
    assert c.screenshot_min_interval_sec == 0.0
...
def test_custom_env(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("VISION_SCREENSHOT_MIN_INTERVAL_SEC", "3")
    monkeypatch.setenv("VISION_SCREENSHOT_MIN_VIDEO_SEC", "8")
    ...
    assert c.screenshot_min_interval_sec == 3.0
    assert c.screenshot_min_video_sec == 8.0
